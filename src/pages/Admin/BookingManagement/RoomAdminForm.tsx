...
import { amenitiesNames, getAmenities } from "src/utils/roomUtil";
...
export default function RoomAdminForm({ room, handleCloseModal }: Props) {
  ...
  const renderRoomLocation = () =>
    room && arrLocation.find((location) => room.maViTri === location.id);
  ...
  const handleSearchLocation = () => {
    return arrLocation?.filter((location) => {
  ...
  const renderLocationList = () =>
    handleSearchLocation()?.map((location: LocationModel) => {
      let locationName =
  ...
  const formik = useFormik<{
    ...
    onSubmit: async (values) => {
      ...
      if (!room) {
        ...
        const addAction = addRoomApi(roomEdit);
      ...
      if (room) {
        ...
        const updateAction = editRoomApi(room?.id, roomEdit);
        ...
        <div className='row mt-3 justify-content-between'>
          <div className='col-6'>
            <div className='form-group mb-3'>
              <label htmlFor='maViTri' className='required'>
                Vị trí
              </label>
              <input
                type='text'
